float vx = 0, vy = 0, vz = 0, sx = 0, sy = 0, sz = 0;
float axOld = 0, ayOld = 0, azOld = 0, vxOld = 0, vyOld = 0, vzOld = 0;
float axThres = 0.005; 
float ayThres = 0.005;
float azThres = 0.1;
float axyThres = 0.002;
float vxy = 0;
...
float ax = 0, ay = 0, az = 0, gx, gy, gz, mx, my, mz; // variables to hold latest sensor data values
...
void loop()
{
  ...
  if( abs(axOld - ax) < axThres) vx = vx; else vx = vx + 0.5 * deltat * (axOld + ax) * 9.81;
  if( abs(ayOld - ay) < ayThres) vy = vy; else vy = vy + 0.5 * deltat * (ayOld + ay) * 9.81;
  if( abs(azOld - az) < azThres) vz = vz; else vz = vz + 0.5 * deltat * (azOld + az - 2) * 9.81;
  if( abs(sqrt(axOld * axOld + ayOld * ayOld) - sqrt(ax * ax + ay * ay)) < axyThres) vxy = vxy; else vxy = vxy + 0.5 * deltat * ( sqrt(axOld * axOld + ayOld * ayOld) + sqrt(ax * ax + ay * ay)) * 9.81;
  ...
  if (!AHRS) {
    ...
    if (delt_t > 25) { // update LCD once per half-second independent of read rate
      ...
      if (SerialDebug) {
        ...
        /*
        Serial.print("runTime = ");Serial.print(runTime); Serial.print(" min\t");

        Serial.print("vxy = "); Serial.print(vxy);
        Serial.print(" vx = "); Serial.print(vx);
        Serial.print(" vy = "); Serial.print(vy);
        Serial.print(" vz = "); Serial.print(vz); Serial.print(" m/s, ");

        Serial.print ("axy = "); Serial.print((int)1000 * sqrt(ax*ax + ay*ay));
     ...
     Serial.print("!");Serial.print(roll,4); Serial.print(",");Serial.print(pitch,4);Serial.print(","); Serial.print(yaw,4);Serial.print("*\t");
